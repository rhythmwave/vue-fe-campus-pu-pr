...
import { GetGradeComponentListData, GetGradeComponentListRoot } from '~~/types/api/grade-component/GetGradeComponentList';
import { ErrorRoot } from '~~/types/api/error';
import { convertDateWithoutTimeToTimezoned } from '~~/utils/helpers/date-utils';
import { CreateGradeComponentRequest } from '~~/types/api/grade-component/CreateGradeComponent';
import { createGradeComponent } from '~~/composables/api/grade-component/createGradeComponent';
...
onMounted(async () => {
    ...
    createGradeComponentRequest.value.study_program_id = adminStudyProgram.value.id
...
const createGradeComponentRequest = ref(<CreateGradeComponentRequest>{});
...
async function save() {
    try {
        const { response: createRoleResponse, error } = await createGradeComponent(createGradeComponentRequest.value)
                    ...
                    <v-row>
                        ...
                        <v-col cols="12" sm="12">
                            <v-select variant="outlined" hide-details :items="subjectCategories" item-value="id"
                                ...
                                v-model="createGradeComponentRequest.subject_category_id"></v-select>

                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-text-field v-model="createGradeComponentRequest.name" variant="outlined" hide-details
                        ...
                        <v-col cols="12" sm="12">
                            <v-select variant="outlined" hide-details item-value="id" item-title="name" label="Status (*)"
                                v-model="createGradeComponentRequest.is_active" :items="[
                                    {
                                        id: true,
                                        name: 'Aktif',
                                    },
                        ...
                        <v-col cols="12">
                            <v-card-actions class="pa-4">
                                ...
                                <v-btn color="secondary" :disabled="createGradeComponentRequest.name == ''"
                                    @click="save(); ">
                                    Tambah
                                </v-btn>
                            </v-card-actions>
