...
import { createLecturerClassWork } from '~~/composables/api/lecturer-class-work/createLecturerClassWork';
...
import { CreateLecturerClassWorkRequest } from '~~/types/api/lecturer-class-work/CreateLecturerClassWork';
...
const createLecturerClassWorkRequest = ref(<CreateLecturerClassWorkRequest>{});
async function save() {
    try {
        createLecturerClassWorkRequest.value.start_time = convertDateTimeToTimezoned(startDate.value, startTime.value);
        createLecturerClassWorkRequest.value.end_time = convertDateTimeToTimezoned(endDate.value, endTime.value);
        // await createFileData()
        createLecturerClassWorkRequest.value.file_path = createFiles.value.path
        createLecturerClassWorkRequest.value.file_path_type = createFiles.value.path_type
        const { response: createRoleResponse, error } = await createLecturerClassWork(createLecturerClassWorkRequest.value)
...
async function lecturerAssignedClassOnChange(id: any) {
    ...
    createLecturerClassWorkRequest.value.class_id = id
}
    ...
    <div>
        <v-card max-height="550" style="overflow: auto;">
            <v-card-title class="pa-4 bg-secondary">
                <span class="title text-white">Tambah Tugas</span>
            </v-card-title>
            <v-card-text>
                <Vspace space="3" />
                <v-form ref="form" lazy-validation>
                    <v-row>
                        ...
                        <v-col cols="12" sm="12">
                            <v-text-field v-model="createLecturerClassWorkRequest.title" variant="outlined" hide-details
                                label="Judul" />
                        </v-col>
                        <v-col cols="12" sm="12">
                        ...
                        <v-col cols="12" sm="12">
                            <v-textarea v-model="createLecturerClassWorkRequest.abstraction" variant="outlined" hide-details
                                label="Abstraksi" />
                        </v-col>
                        <v-col cols="12" sm="6">
