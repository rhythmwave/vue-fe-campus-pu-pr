...
import { createLecturerClassMaterial } from '~~/composables/api/lecturer-class-material/createLecturerClassMaterial';
...
import { CreateLecturerClassMaterialRequest } from '~~/types/api/lecturer-class-material/CreateLecturerClassMaterial';
...
const createLecturerClassMaterialRequest = ref(<CreateLecturerClassMaterialRequest>{});
async function save() {
    try {
        // await createFileData()
        createLecturerClassMaterialRequest.value.file_path = createFiles.value.path
        createLecturerClassMaterialRequest.value.file_path_type = createFiles.value.path_type
        const { response: createRoleResponse, error } = await createLecturerClassMaterial(createLecturerClassMaterialRequest.value)
...
async function lecturerAssignedClassOnChange(id: any) {
    ...
    createLecturerClassMaterialRequest.value.class_id = id
}
    ...
    <div>
        <v-card max-height="550" style="overflow: auto;">
            <v-card-title class="pa-4 bg-secondary">
                <span class="title text-white">Tambah Materi</span>
            </v-card-title>
            <v-card-text>
                <Vspace space="3" />
                <v-form ref="form" lazy-validation>
                    <v-row>
                        ...
                        <v-col cols="12" sm="12">
                            <v-text-field v-model="createLecturerClassMaterialRequest.title" variant="outlined" hide-details
                                label="Judul" />
                        </v-col>
                        <v-col cols="12" sm="12">
                        ...
                        <v-col cols="12" sm="12">
                            <v-textarea v-model="createLecturerClassMaterialRequest.abstraction" variant="outlined" hide-details
                                label="Abstraksi" />
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-switch v-model="createLecturerClassMaterialRequest.is_active" label="Aktif" color="primary"></v-switch>
                        </v-col>
                        <v-col cols="12">
