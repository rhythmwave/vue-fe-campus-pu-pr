...
import { SetClassGradeComponentData, SetClassGradeComponentRequest } from "~~/types/api/class-grade-component/SetClassGradeComponent";
import { setClassGradeComponent } from '~~/composables/api/class-grade-component/setClassGradeComponent';
...
const createClassGradeComponent = ref(<SetClassGradeComponentRequest>{})
...
watch(props.input, value => {
    ...
    createClassGradeComponent.value = {
        class_id: <string>props.class_id,
        data: <SetClassGradeComponentData[]>(objects),
    }
    console.log("objects")
    console.log(objects)
...
async function deleteClassGradeComponent(item: any) {
    let index = createClassGradeComponent.value.data.findIndex(x => x.name === item.name)
    createClassGradeComponent.value.data.splice(index, 1)
}
async function save() {
    let sumPercentage = 0
    var tempData = <SetClassGradeComponentData[]>[];
    var isExist = false;
    var isHaveNonames = false;
    createClassGradeComponent.value?.data.forEach(function (item) {
        item.is_active = true
    ...
    try {
        const { response: resp, error: error } = await setClassGradeComponent(createClassGradeComponent.value)
        if (error) {
                ...
                <v-row>
                    <v-col>
                        <v-table>
                            ...
                            <tbody>
                                <tr v-for="(item, index) in createClassGradeComponent?.data">
                                    <!-- <td>
                                    ...
                                    <td>
                                        <v-btn color="warning" @click="deleteClassGradeComponent(item);">
                                            Delete
                                        </v-btn>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                        <v-btn color="secondary" @click="createClassGradeComponent?.data.push({
                                            name: '',
                                            is_active: true,
                                            percentage: 0,
                                        })">
