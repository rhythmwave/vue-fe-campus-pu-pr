...
import { UpdateSubjectGradeComponent, SubjectGradeComponentDataList } from "~~/types/api/subject-grade-component/UpdateSubjectGradeComponent";
import { setSubjectGradeComponent } from '~~/composables/api/grade-component/setSubjectGradeComponent';
...
const createSubjectGradeComponent = ref(<UpdateSubjectGradeComponent>{})
...
watch(props.input, value => {
    ...
    createSubjectGradeComponent.value = {
        subject_id: <string>props.subject_id,
        data: <SubjectGradeComponentDataList[]>(objects),
    }
    console.log("objects")
    console.log(objects)
...
async function deleteSubjectGradeComponent(item: any) {
    let index = createSubjectGradeComponent.value.data.findIndex(x => x.name === item.name)
    createSubjectGradeComponent.value.data.splice(index, 1)
}
async function save() {
    let sumPercentage = 0
    var tempData = <SubjectGradeComponentDataList[]>[];
    var isExist = false;
    var isHaveNonames = false;
    createSubjectGradeComponent.value?.data.forEach(function (item) {
        item.is_active = true
    ...
    try {
        const { response: resp, error: error } = await setSubjectGradeComponent(createSubjectGradeComponent.value)
        if (error) {
                ...
                <v-row>
                    <v-col>
                        <v-table>
                            ...
                            <tbody>
                                <tr v-for="(item, index) in createSubjectGradeComponent?.data">
                                    <!-- <td>
                                    ...
                                    <td>
                                        <v-btn color="warning" @click="deleteSubjectGradeComponent(item);">
                                            Delete
                                        </v-btn>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                        <v-btn color="secondary" @click="createSubjectGradeComponent?.data.push({
                                            name: '',
                                            is_active: true,
                                            percentage: 0,
                                        })">
