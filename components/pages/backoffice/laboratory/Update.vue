...
import { updateLaboratory } from '~~/composables/api/laboratory/updateLaboratory';
import { ErrorRoot } from '~~/types/api/error';
import { UpdateLaboratoryRequest } from '~~/types/api/laboratory/UpdateLaboratory';
...
onMounted(async () => {
    ...
    detailLaboratory = <GetLaboratoryListData>props.getLaboratoryListData
    updateLaboratoryRequest.value = Object.assign({}, <UpdateLaboratoryRequest>props.getLaboratoryListData)
    await getLaboratoryDetailData(detailLaboratory.id)
    updateLaboratoryRequest.value = Object.assign({}, <UpdateLaboratoryRequest>laboratoryDetailData.value)
...
const updateLaboratoryRequest = ref(<UpdateLaboratoryRequest>{});
let detailLaboratory: GetLaboratoryListData;

const emit = defineEmits(['close', 'reload'])

async function update() {
    try {
        const { response: updateRoleResponse, error } = await updateLaboratory(updateLaboratoryRequest.value)
    ...
    <div>
        <v-card max-height="600" style="overflow: auto;">
            <v-card-title class="pa-4 bg-secondary">
                <span class="title text-white">Ubah Laboratorium</span>
            </v-card-title>
            <v-card-text>
                <Vspace space="3" />
                <v-form ref="form" v-model="valid" lazy-validation>
                    <v-row>
                        ...
                        <v-col cols="12" sm="12">
                            <v-text-field v-model="updateLaboratoryRequest.code" variant="outlined" hide-details
                                label="Kode Laboratorium(*)" :readonly="false" />
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-text-field v-model="updateLaboratoryRequest.name" variant="outlined" hide-details
                                label="Nama Laboratorium" :readonly="false" />
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-text-field v-model.number="updateLaboratoryRequest.capacity" type="number" variant="outlined"
                                hide-details label="Kapasitas" :readonly="false" />
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-text-field v-model.number="updateLaboratoryRequest.exam_capacity" type="number"
                                variant="outlined" hide-details label="Kapasitas Ujian" :readonly="false" />
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-label text="Status Kelayakan"></v-label>
                            <v-radio-group v-model="updateLaboratoryRequest.is_usable" inline hide-details="auto">
                        ...
                        <v-col cols="12" sm="12">
                            <v-text-field v-model.number="updateLaboratoryRequest.area" type="number" variant="outlined"
                                hide-details label="Luas(m2)" :readonly="false" />
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-text-field v-model="updateLaboratoryRequest.phone_number" variant="outlined" hide-details
                                label="Telepon" :readonly="false" />
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-textarea outlined v-model="updateLaboratoryRequest.facility" label="Fasilitas"
                                variant="outlined" hide-details></v-textarea>
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-textarea outlined v-model="updateLaboratoryRequest.remarks" label="Catatan"
                                variant="outlined" hide-details></v-textarea>
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-select variant="outlined" hide-details :items="owners" item-value="id" item-title="name"
                                label="Kepemilikan" v-model="updateLaboratoryRequest.owner"></v-select>
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-select variant="outlined" hide-details :items="locations" item-value="id" item-title="name"
                                label="Lokasi" v-model="updateLaboratoryRequest.location"></v-select>
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-select variant="outlined" hide-details :items="functions" item-value="id" item-title="name"
                                label="Fungsi" v-model="updateLaboratoryRequest.purpose"></v-select>
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-select variant="outlined" hide-details :items="studyPrograms" item-value="id"
                                item-title="name" label="Program Studi(*)"
                                v-model="updateLaboratoryRequest.study_program_id"></v-select>
                        </v-col>
                        <v-col cols="12">
                            <v-card-actions class="pa-4">
                                ...
                                <v-btn color="secondary" :disabled="updateLaboratoryRequest.name == ''" @click="update(); ">
                                    Ubah
                                </v-btn>
                            </v-card-actions>
                        </v-col>
