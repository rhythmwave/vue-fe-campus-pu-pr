...
import { GetLecturerResignationListData, GetLecturerResignationListRoot } from '~~/types/api/lecturer-resignation/GetLecturerResignationList';
import { ErrorRoot } from '~~/types/api/error';
import { convertDateWithoutTimeToTimezoned } from '~~/utils/helpers/date-utils';
import { CreateLecturerResignationRequest } from '~~/types/api/lecturer-resignation/CreateLecturerResignation';
import { createLecturerResignation } from '~~/composables/api/lecturer-resignation/createLecturerResignation';
...
onMounted(async () => {
    adminStudyProgramData.value = <GetAdminStudyProgramListData>props.adminStudyProgram
    semester.value = <GetSemesterListData>props.semester
    createLecturerResignationRequest.value.semester_id = semester.value.id
    await getLecturers()
});
...
const createLecturerResignationRequest = ref(<CreateLecturerResignationRequest>{
    lecturer_id: '',
    semester_id: '',
    resign_date: '',
    resignation_number: '',
...
async function save() {
    try {
        let req: CreateLecturerResignationRequest = Object.assign({}, createLecturerResignationRequest.value)
        if (req.resign_date) {
            req.resign_date = convertDateWithoutTimeToTimezoned(req.resign_date)
        }
        const { response: createRoleResponse, error } = await createLecturerResignation(req)
...
async function getLecturers() {
    try {
        const { response: resp, error: error } = await getLecturerList(1000000, 1, "", adminStudyProgramData.value.id, "", "", semester.value.id, '', '', '', '', '')
        if (error) {
            let dataError = <ErrorRoot>error;
            alert(dataError.meta.message);
            return;
...
function saveLecturer() {
    ...
    createLecturerResignationRequest.value.lecturer_id = lecturersSelected.value.id;
}
    ...
    <div>
        <v-card max-height="600" style="overflow: auto;">
            <v-card-title class="pa-4 bg-secondary">
                <span class="title text-white">Tambah Dosen Keluar</span>
            </v-card-title>
            <v-card-text>
                <Vspace space="3" />
                <v-form ref="form" v-model="valid" lazy-validation>
                    <v-row>
                        ...
                        <v-col cols="12" sm="12">
                            <v-text-field v-model="semester.semester_type_year" variant="outlined" hide-details label="Semester"
                                :readonly="true" />
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-text-field v-model="createLecturerResignationRequest.resign_date" type="date"
                                variant="outlined" hide-details label="Tanggal" :readonly="false" />
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-text-field v-model="createLecturerResignationRequest.resignation_number" variant="outlined"
                                hide-details label="Nomor Surat" :readonly="false" />
                        </v-col>
                        <v-col cols="12" sm="12">

                            <v-select :items="['KELUAR',
                                'ALMARHUM',
                                'PENSIUN'
                            ]" label="Purpose" density="comfortable"
                                v-model="createLecturerResignationRequest.purpose"></v-select>
                        </v-col>
                        <!-- <v-col cols="12" sm="12">
                        ...
                        <v-col cols="12" sm="12">
                            <v-textarea outlined v-model="createLecturerResignationRequest.remarks" label="Catatan"
                        ...
                        <v-col cols="12" sm="12">
                            <v-select variant="outlined" hide-details :items="Sample" item-value="id"
                                ...
                                v-model="createLecturerResignationRequest.name"></v-select>
                        </v-col> -->
                        <v-col cols="12">
                            <v-card-actions class="pa-4">
                                ...
                                <v-btn color="secondary" :disabled="createLecturerResignationRequest.lecturer_id == ''"
                                    @click="save();">
                                    Tambah
                                </v-btn>
                            </v-card-actions>
