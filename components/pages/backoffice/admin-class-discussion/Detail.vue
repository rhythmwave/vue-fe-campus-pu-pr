...
const classDiscussions = ref<GetClassDiscussionListData[]>([])
async function getData(limit: number, page: number) {
    try {
        const { response: resp, error: error } = await getClassDiscussionList(limit, page, search.value, detailAdminClass.value.id)
        if (error) {
        ...
        if (resp != null) {
            classDiscussions.value = resp?.data
...
<template>
    <v-card>
        <v-card-title class="pa-4 bg-secondary">
            <span class="title text-white">Diskusi Kelas</span>
        </v-card-title>
                ...
                <UiParentCard title="Pencarian">
                    <v-row>
                        <v-col cols="12" lg="4" md="6">
                            <v-text-field density="compact" v-model="search" label="Pencarian Diskusi Kelas" hide-details
            ...
            <v-col cols="12"><v-table class="mt-5 pl-5 pr-5">
                    <thead>
                        <tr>
                            ...
                            <th class="text-subtitle-1 font-weight-semibold">Judul Topik</th>
                            <th class="text-subtitle-1 font-weight-semibold">Moderator</th>
                            <th class="text-subtitle-1 font-weight-semibold">Jumlah Pesan Diskusi</th>
                            <th class="text-subtitle-1 font-weight-semibold">Diskusi Terakhir</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="item, index in classDiscussions" :key="item.id">
