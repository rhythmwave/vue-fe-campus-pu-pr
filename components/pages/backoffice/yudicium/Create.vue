...
import { CreateYudiciumRequest } from '~~/types/api/yudicium/CreateYudicium';
import { createYudicium } from '~~/composables/api/yudicium/createYudicium';
...
onMounted(async () => {
    ...
    createYudiciumRequest.value.study_program_id = sample.value.id
...
const createYudiciumRequest = ref(<CreateYudiciumRequest>{});
...
async function save() {
    try {
        const { response: createRoleResponse, error } = await createYudicium(createYudiciumRequest.value)
    ...
    <div>
        <v-card max-height="600" style="overflow: auto;">
            <v-card-title class="pa-4 bg-secondary">
                <span class="title text-white">Tambah Pendaftaran Yudisium</span>
            </v-card-title>
            <v-card-text>
                <Vspace space="3" />
                    ...
                    <v-row>
                        ...
                        <v-col cols="12" sm="12">
                            <v-select variant="outlined" hide-details :items="adminStudyProgram" item-value="id"
                                ...
                                v-model="createYudiciumRequest.name"></v-select>
                        </v-col>
                        <v-col cols="12">
                            <v-card-actions class="pa-4">
                                ...
                                <v-btn color="secondary" :disabled="createYudiciumRequest.name == ''"
                                    @click="save(); ">
                                    Tambah
                                </v-btn>
                            </v-card-actions>
