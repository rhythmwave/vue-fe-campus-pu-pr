...
import { GetClassWorkListData } from '~~/types/api/class-work/GetClassWorkList';
...
import { getClassWorkList } from '~~/composables/api/class-work/getClassWorkList';
...
const classWorks = ref<GetClassWorkListData[]>([])
async function getData(limit: number, page: number) {
    try {
        const { response: resp, error: error } = await getClassWorkList(limit, page, search.value, detailAdminClass.value.id)
        if (error) {
...
<template>
    <v-card>
        <v-card-title class="pa-4 bg-secondary">
            <span class="title text-white">Ubah Tugas Kuliah</span>
        </v-card-title>
            ...
            <v-col cols="12"><v-table class="mt-5 pl-5 pr-5">
                    <thead>
                        <tr>
                            ...
                            <th class="text-subtitle-1 font-weight-semibold">Dwonload Tugas</th>
                            ...
                            <th class="text-subtitle-1 font-weight-semibold">Tugas Terkumpul</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="item, index in classWorks" :key="item.id">
                            <td class="text-subtitle-1">{{ index + 1 }}</td>
                            <td><a :href="item.file_url" download="">Download</a></td>
                            <td>{{ item.title }}</td>
                            <td>{{ item.abstraction }}</td>
                            <td>{{ item.lecturer_name }}</td>
                            <td>{{ moment.duration(moment(item.end_time).diff(moment(item.start_time))).asMinutes() }} Menit</td>
