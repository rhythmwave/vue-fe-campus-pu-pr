...
const classEvents = ref<GetClassEventListData[]>([])
async function getData(limit: number, page: number) {
    try {
        const { response: resp, error: error } = await getClassEventList(limit, page, search.value, detailAdminClass.value.id)
        if (error) {
...
<template>
    <v-card>
        <v-card-title class="pa-4 bg-secondary">
            <span class="title text-white">Agenda Kelas</span>
        </v-card-title>
            ...
            <v-col cols="12"><v-table class="mt-5 pl-5 pr-5">
                    <thead>
                        <tr>
                            ...
                            <th class="text-subtitle-1 font-weight-semibold">Judul Agenda</th>
                            ...
                            <th class="text-subtitle-1 font-weight-semibold">Frekuensi</th>
                            <th class="text-subtitle-1 font-weight-semibold">Waktu</th>
                            <th class="text-subtitle-1 font-weight-semibold">Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="item, index in classEvents" :key="item.id">
                            <td class="text-subtitle-1">{{ index + 1 }}</td>
                            <td>{{ item.title }}</td>
                            <td>{{ item.remarks }}</td>
                            <td>{{ item.lecturer_name }}</td>
                            <td>{{ item.frequency }}</td>
                            <td>{{ moment(item.event_time ).format('DD MMM YYYY HH:mm') }}</td>
