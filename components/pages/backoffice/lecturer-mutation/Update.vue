...
import { updateLecturerMutation } from '~~/composables/api/lecturer-mutation/updateLecturerMutation';
...
import { UpdateLecturerMutationRequest } from '~~/types/api/lecturer-mutation/UpdateLecturerMutation';
...
const props = defineProps({
    sample: Array as any,
    getLecturerMutationListData: Array as any,
});

onMounted(async () => {
    sample.value = Object.assign({}, <GetSampleListData>props.sample)
    detailLecturerMutation = <GetLecturerMutationListData>props.getLecturerMutationListData
    updateLecturerMutationRequest.value = Object.assign({}, <UpdateLecturerMutationRequest>props.getLecturerMutationListData)
    updateLecturerMutationRequest.value.study_program_id = sample.value.id
    await getSemester()
});
...
const updateLecturerMutationRequest = ref(<UpdateLecturerMutationRequest>{});
let detailLecturerMutation: GetLecturerMutationListData;
...
async function update() {
    try {
        const { response: updateRoleResponse, error } = await updateLecturerMutation(updateLecturerMutationRequest.value)
            ...
            <v-card-text>
                ...
                <v-form ref="form" v-model="valid" lazy-validation>
                    <v-row>
                        ...
                        <v-col cols="12" sm="12">
                            <!-- <v-select variant="outlined" hide-details :items="semester" item-value="id"
                                item-title="name" label="Options"
                                v-model="updateLecturerMutationRequest.name"></v-select> -->
                            <v-select v-model="updateLecturerMutationRequest.semester_id" item-value="id" variant="outlined" hide-details
                                :items="semester" label="Pilih Semester">
                        ...
                        <v-col cols="12" sm="12">
                            <v-select variant="outlined" hide-details item-value="id" item-title="name" label="Status (*)"
                                v-model="updateLecturerMutationRequest.is_active" :items="[
                                    {
                                        id: true,
                                        name: 'Aktif',
                                    },
                        ...
                        <v-col cols="12">
                            <v-card-actions class="pa-4">
                                ...
                                <v-btn color="secondary" :disabled="updateLecturerMutationRequest.name == ''"
                                    @click="update();">
                                    Ubah
                                </v-btn>
                            </v-card-actions>
