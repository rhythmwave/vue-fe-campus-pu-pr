...
import { GetLecturerMutationListData, GetLecturerMutationListRoot } from '~~/types/api/lecturer-mutation/GetLecturerMutationList';
import { ErrorRoot } from '~~/types/api/error';
import { convertDateWithoutTimeToTimezoned } from '~~/utils/helpers/date-utils';
import { CreateLecturerMutationRequest } from '~~/types/api/lecturer-mutation/CreateLecturerMutation';
import { createLecturerMutation } from '~~/composables/api/lecturer-mutation/createLecturerMutation';
...
onMounted(async () => {
    ...
    createLecturerMutationRequest.value.semester_id = semester.value.id
    await getLecturers()
});
...
const createLecturerMutationRequest = ref(<CreateLecturerMutationRequest>{
    lecturer_id: '',
    semester_id: '',
    mutation_date: '',
    decision_number: '',
...
async function save() {
    try {
        let req: CreateLecturerMutationRequest = Object.assign({}, createLecturerMutationRequest.value)
        if (req.mutation_date) {
            req.mutation_date = convertDateWithoutTimeToTimezoned(req.mutation_date)
        }
        const { response: createRoleResponse, error } = await createLecturerMutation(req)
...
function saveLecturer() {
    ...
    createLecturerMutationRequest.value.lecturer_id = lecturersSelected.value.id;
}
    ...
    <div>
        <v-card max-height="600" style="overflow: auto;">
            <v-card-title class="pa-4 bg-secondary">
                <span class="title text-white">Tambah Dosen Tugas Ke Instansi Lain</span>
            </v-card-title>
            <v-card-text>
                <Vspace space="3" />
                <v-form ref="form" v-model="valid" lazy-validation>
                    <v-row>
                        ...
                        <v-col cols="12" sm="12">
                            <v-text-field v-model="createLecturerMutationRequest.mutation_date" type="date"
                                variant="outlined" hide-details label="Tanggal" :readonly="false" />
                        </v-col>
                        <v-col cols="12" sm="12">
                            <v-text-field v-model="createLecturerMutationRequest.decision_number" variant="outlined"
                                hide-details label="Nomor Keputusan" :readonly="false" />
                        </v-col>
                        <!-- <v-col cols="12" sm="12">
                            ...
                            <v-select :items="['KELUAR',
                                ...
                                v-model="createLecturerMutationRequest.purpose"></v-select>
                        </v-col> -->
                        <!-- <v-col cols="12" sm="12">
                        ...
                        <v-col cols="12" sm="12">
                            <v-textarea outlined v-model="createLecturerMutationRequest.destination" label="Tujuan"
                        ...
                        <v-col cols="12" sm="12">
                            <v-select variant="outlined" hide-details :items="Sample" item-value="id"
                                ...
                                v-model="createLecturerMutationRequest.name"></v-select>
                        </v-col> -->
                        <v-col cols="12">
                            <v-card-actions class="pa-4">
                                ...
                                <v-btn color="secondary" :disabled="createLecturerMutationRequest.lecturer_id == ''"
                                    @click="save();">
                                    Tambah
                                </v-btn>
                            </v-card-actions>
