...
import { GetAcademicPortalActivityLogListData } from '~~/types/api/academic-portal-activity-log/GetAcademicPortalActivityLog';
...
const page = ref({ title: 'Log Aktifitas Portal' });
...
const adminLogList = ref(<GetAcademicPortalActivityLogListData[]>[]);
...
async function getData(limit: number, page: number, search: string, month: number, year: number) {
    try {
        const { data: getAdminActivityLogList, pending, refresh, error } = await useAPIFetch<any>(() => `/api/v1/root/root_lecturer_student_activity_log.RootLecturerStudentActivityLogHandler/GetList?limit=${limit}&page=${page}&search=${search}&month=${month}&year=${year}`, <any>{
            method: 'GET',
        })
        if (error.value) {
        ...
        adminLogList.value = <GetAcademicPortalActivityLogListData[]>getAdminActivityLogList.value?.data;
...
<template>
    <div>
        ...
        <v-row>
            <v-col cols="12">
                <SharedUiParentCard title="Log Aktifitas Portal">
                    <v-row>
        ...
        <v-table class="mt-5">
            <thead>
                <tr>
                    ...
                    <th class="text-subtitle-1 font-weight-semibold">User Username</th>
                    <th class="text-subtitle-1 font-weight-semibold">User Type</th>
            ...
            <tbody>
                <tr v-for="item, index in adminLogList" :key="item.id">
                    ...
                    <td class="text-subtitle-1">{{ item.user_name }}</td>
                    <td class="text-subtitle-1">{{ item.user_username }}</td>
                    <td class="text-subtitle-1">{{ item.user_type }}</td>
