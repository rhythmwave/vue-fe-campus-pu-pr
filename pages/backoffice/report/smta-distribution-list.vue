...
import { GetReportStudentSchoolProvinceListData } from '~~/types/api/report/GetReportStudentSchoolProvinceList';
import { getReportStudentSchoolProvinceList } from '~~/composables/api/report/getReportStudentSchoolProvinceList';
...
const page = ref({ title: 'Daftar Distribusi Asal SMTA Per Provinsi' });
const breadcrumbs = ref([
    ...
    {
        text: 'Daftar Distribusi Asal SMTA Per Provinsi',
        disabled: true,
        href: '#'
    }
]);
...
async function studyProgramOnChange(id: any) {
    ...
    await getReportStudentSchoolProvinceData()
}

const reportStudentSchoolProvinces = ref<GetReportStudentSchoolProvinceListData[]>([])
const selectedReportStudentSchoolProvince = ref(<GetReportStudentSchoolProvinceListData>{});
async function getReportStudentSchoolProvinceData() {
    try {
        if (study_program_id.value == '' || student_force_from.value == '' || student_force_to.value == '') {
            reportStudentSchoolProvinces.value = []
            return;
        }
        const { response: resp, error: error } = await getReportStudentSchoolProvinceList(study_program_id.value, student_force_from.value, student_force_to.value)
        if (error) {
            let dataError = <ErrorRoot>error;
            alert(dataError.meta.message);
            return;
        ...
        if (resp != null) {
            reportStudentSchoolProvinces.value = resp?.data
        }
    } catch (error) {
        alert(error)
    }
...
<template>
    <div>
                    ...
                    <v-row>
                        ...
                        <v-col cols="12" sm="12" lg="4" md="6">
                            <v-select variant="outlined" hide-details :items="studentYears" item-value="id"
                                ...
                                @update:model-value="(c: any) => getReportStudentSchoolProvinceData()"></v-select>
                        </v-col>
                        <v-col cols="12" sm="12" lg="4" md="6">
                            <v-select variant="outlined" hide-details :items="studentYears" item-value="id"
                        ...
                        <v-col cols="12" lg="12" md="12" class="text-right">
                            <v-btn color="primary" v-bind="props" flat class="ml-auto" target="_blank"
                                :to="'/backoffice/report/prints/smta-distribution-list/' + study_program_id + '?from='+student_force_from+'&to='+student_force_to"
                                :disabled="!reportStudentSchoolProvinces.length">
        ...
        <v-table class="mt-5" v-if="reportStudentSchoolProvinces.length">
            <thead>
                <tr>
                    <th class="text-subtitle-1 font-weight-semibold">No</th>
                    <th class="text-subtitle-1 font-weight-semibold">Aksi</th>
                    <th class="text-subtitle-1 font-weight-semibold">Provinsi</th>
                    <th v-for="sf in reportStudentSchoolProvinces[0].student_forces">
                        {{ sf.student_force }}
                    </th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="item, i in reportStudentSchoolProvinces">
