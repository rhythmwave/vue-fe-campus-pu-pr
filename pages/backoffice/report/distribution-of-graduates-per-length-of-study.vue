...
import { GetReportStudyDurationListData, GetReportStudyDurationListRequest } from '~~/types/api/report/GetReportStudyDurationList';
import { getReportStudyDurationList } from '~~/composables/api/report/getReportStudyDurationList';
...
const page = ref({ title: 'Daftar Distribusi berdasarkan Lama Study'});
const breadcrumbs = ref([
    ...
    {
        text: 'Daftar Distribusi berdasarkan Lama Study',
        disabled: true,
        href: '#'
    }
]);
...
async function studyProgramOnChange(id: any) {
    ...
    await getReportStudyDurationData()
}

const reportStudyDurations = ref<GetReportStudyDurationListData[]>([])
const selectedReportStudyDuration = ref(<GetReportStudyDurationListData>{});
async function getReportStudyDurationData() {
    try {
        if (study_program_id.value == '' || student_graduation_from.value == '' || student_graduation_to.value == '') {
            reportStudyDurations.value = []
            return;
        }
        const { response: resp, error: error } = await getReportStudyDurationList(<GetReportStudyDurationListRequest>{
        ...
        if (resp != null) {
            reportStudyDurations.value = resp?.data
        }
    } catch (error) {
        alert(error)
    }
...
<template>
    <div>
                    ...
                    <v-row>
                        ...
                        <v-col cols="12" sm="12" lg="4" md="6">
                            <v-select variant="outlined" hide-details :items="studentYears" item-value="id"
                                ...
                                @update:model-value="(c: any) => getReportStudyDurationData()"></v-select>
                        </v-col>
                        <v-col cols="12" sm="12" lg="4" md="6">
                            <v-select variant="outlined" hide-details :items="studentYears" item-value="id"
                        ...
                        <v-col cols="12" lg="12" md="12" class="text-right">
                            <v-btn color="primary" v-bind="props" flat class="ml-auto" target="_blank"
                                :to="'/backoffice/report/prints/distribution-of-graduates-per-length-of-study   /' + study_program_id + '?from=' + student_graduation_from + '&to=' + student_graduation_to"
                                :disabled="!reportStudyDurations.length">
        ...
        <v-table class="mt-5" v-if="reportStudyDurations.length">
            <thead>
                <tr>
                    <th class="text-subtitle-1 font-weight-semibold">No</th>
                    <th class="text-subtitle-1 font-weight-semibold">Tahun Lulus</th>
                    <th class="text-subtitle-1 font-weight-semibold">&lt; 3 Tahun</th>
                    <th class="text-subtitle-1 font-weight-semibold">%</th>
                    <th class="text-subtitle-1 font-weight-semibold">3 - 3,5 Tahun</th>
                    <th class="text-subtitle-1 font-weight-semibold">%</th>
                    <th class="text-subtitle-1 font-weight-semibold">3,5 - 4 Tahun</th>
                    <th class="text-subtitle-1 font-weight-semibold">%</th>
                    <th class="text-subtitle-1 font-weight-semibold"> > 4 Tahun</th>
                    <th class="text-subtitle-1 font-weight-semibold">%</th>
                    <th class="text-subtitle-1 font-weight-semibold">Total Lulusan</th>
                    <th class="text-subtitle-1 font-weight-semibold">Lama Studi Rata Rata</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="item, i in reportStudyDurations">
                    <td class="text-subtitle-1">{{ i + 1 }}</td>
                    <td class="text-subtitle-1">{{ item.graduation_school_year }}</td>
                    <td class="text-subtitle-1">{{ item.total_less_than_3 }}</td>
                    <td class="text-subtitle-1">{{ item.percentage_less_than_3 }}</td>
                    <td class="text-subtitle-1">{{ item.total_between_3_and_35 }}</td>
                    <td class="text-subtitle-1">{{ item.total_between_3_and_35 }}</td>
                    <td class="text-subtitle-1">{{ item.total_between_35_and_4 }}</td>
                    <td class="text-subtitle-1">{{ item.total_between_35_and_4 }}</td>
                    <td class="text-subtitle-1">{{ item.total_more_than_4 }}</td>
                    <td class="text-subtitle-1">{{ item.percentage_more_than_4 }}</td>
                    <td class="text-subtitle-1">{{ item.total_graduates }}</td>
                    <td class="text-subtitle-1">{{ item.average_study_duration }}</td>
                </tr>
            </tbody>
        </v-table>
    </div>
