...
async function getClassroomListData() {
    try {
        const { response: resp, error: error } = await getClassroomList(100000, 1, '', study_program_id.value, semester_id.value, true, '', false)
        if (error) {
            let dataError = <ErrorRoot>error;
            alert(dataError.meta.message);
            return;
            ...
            <div class="page-break">
                ...
                <table width="100%" class="tabel-common">
                    <tbody>
                        ...
                        <tr v-for="cl, i in classroomLists">
                            <td align="right">{{ i + 1 }}</td>
                            <td nowrap="nowrap" align="left">{{ cl.name }}</td>
                            <td nowrap="nowrap" align="left">
                                <div style="width: 300; overflow: hidden;">{{ cl.subject_name }}</div>
                            </td>
                            <td align="center">{{ cl.total_participant }}</td>
                            <td align="center">{{ cl.maximum_participant }}</td>
                        </tr>
                ...
                <table align="right" width="100%">
                    <tbody>
                        <tr>
                            ...
                            <td align="center">Semarang, {{ new Date().getDate() }}, {{ convertMonthToTextString(new Date().getMonth() + 1) }} {{ new Date().getFullYear() }}<br><span
