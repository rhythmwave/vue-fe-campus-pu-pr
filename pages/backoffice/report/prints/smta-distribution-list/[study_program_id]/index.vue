...
onMounted(async () => {
    ...
    await getReportStudentSchoolProvinceListData()
    await delay(1000)
    window.print()
})
...
const reportStudentSchoolProvinceLists = ref<GetReportStudentSchoolProvinceListData[]>([])
async function getReportStudentSchoolProvinceListData() {
    try {
        const { response: resp, error: error } = await getReportStudentSchoolProvinceList(study_program_id.value, from.value, to.value)
        if (error) {
            let dataError = <ErrorRoot>error;
            alert(dataError.meta.message);
            return;
        ...
        if (resp != null) {
            reportStudentSchoolProvinceLists.value = resp?.data
        }
    } catch (error) {
        alert(error)
    }
        ...
        <div class="nobreak">
            ...
            <div class="page-break">
                ...
                <div class="header">
                    ...
                    <h2 align="center">DISTRIBUSI ASAL SMTA MAHASISWA PER PROPINSI</h2>
                ...
                <table class="tabel-common" align="center" width="100%" v-if="reportStudentSchoolProvinceLists.length != 0">
                    <tbody>
                        <tr align="center">
                            <th rowspan="2">No.</th>
                            ...
                            <th :colspan="reportStudentSchoolProvinceLists[0].student_forces.length" align="center">Tahun</th>
                        </tr>
                        <tr>
                            <th v-for="sf in reportStudentSchoolProvinceLists[0].student_forces"><strong>{{ sf.student_force
                            }}</strong></th>
                        </tr>
                        <tr align="center" v-for="rsp, i in reportStudentSchoolProvinceLists">
                            <td>
                                {{ i + 1 }}
                            </td>
                        ...
                        <tr>
                            ...
                            <td align="center"
                                v-for="sf in reportStudentSchoolProvinceLists[0].student_forces.map((sf) => sf.student_force)">
                                <b>
                                    {{
                                        reportStudentSchoolProvinceLists.map((rsp) => rsp.student_forces.filter((s) => s.student_force == sf)[0].total).reduce((a, b) => a + b)
                                    }}
                                    <b>
                                    </b></b>
                            </td>
