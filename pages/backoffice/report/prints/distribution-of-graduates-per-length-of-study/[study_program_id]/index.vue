...
onMounted(async () => {
    ...
    await getReportStudyDurationListData()
    await delay(1000)
    window.print()
})
...
const reportStudyDurationLists = ref<GetReportStudyDurationListData[]>([])
async function getReportStudyDurationListData() {
    try {
        const { response: resp, error: error } = await getReportStudyDurationList(<GetReportStudyDurationListRequest>{
            ...
            graduation_year_from: from.value,
            graduation_year_to: to.value,
        })
        if (error) {
            let dataError = <ErrorRoot>error;
            alert(dataError.meta.message);
        ...
        if (resp != null) {
            reportStudyDurationLists.value = resp?.data
        }
    } catch (error) {
        alert(error)
    }
        ...
        <div class="nobreak">
            ...
            <div class="page-break">
                ...
                <div class="header">
                    ...
                    <h2 align="center">DISTRIBUSI LULUSAN BERDASARKAN LAMA STUDI PER TAHUN AJARAN </h2>
                ...
                <table class="tabel-common" align="center" width="100%" v-if="reportStudyDurationLists.length != 0">
                    <tbody>
                        <tr align="center" valign="middle">
                            <th rowspan="3">No</th>
                            <th rowspan="3">Tahun<br>Lulus</th>
                            <th colspan="8" nowrap="nowrap">Lama Studi</th>
                            <th rowspan="3">Total<br>Lulusan</th>
                            <th nowrap="nowrap" rowspan="3">Lama Studi<br>Rata-rata</th>
                        </tr>
                        <tr align="center">
                            <th colspan="2" nowrap="nowrap"> &lt;&nbsp;3&nbsp; Tahun</th>
                            <th colspan="2" nowrap="nowrap"> 3 - 3.5 Tahun</th>
                            <th colspan="2" nowrap="nowrap"> 3.5 - 4 Tahun</th>
                            <th colspan="2" nowrap="nowrap"> &gt;&nbsp;4 Tahun</th>
                        </tr>
                        <tr align="center">
                            <th>Jumlah</th>
                            <th>%</th>
                        ...
                        <tr align="center" v-for="item,index in reportStudyDurationLists">
                            <td nowrap="nowrap">{{ index+1 }}</td>
                            <td nowrap="nowrap">{{ item.graduation_school_year }}</td>
                            <td nowrap="nowrap">{{ item.total_less_than_3 }}</td>
                            <td nowrap="nowrap">{{ item.percentage_less_than_3 }}</td>
                            <td nowrap="nowrap">{{ item.total_between_3_and_35 }}</td>
                            <td nowrap="nowrap">{{ item.percentage_between_3_and_35 }}</td>
                            <td nowrap="nowrap">{{ item.total_between_35_and_4 }}</td>
                            <td nowrap="nowrap">{{ item.percentage_between_35_and_4 }}</td>
                            <td nowrap="nowrap">{{ item.total_more_than_4 }}</td>
                            <td nowrap="nowrap">{{ item.percentage_more_than_4 }}</td>
                            <td nowrap="nowrap">{{ item.total_graduates }}</td>
                            <td nowrap="nowrap">{{ item.average_study_duration }}</td>
                        </tr>
                    </tbody>
                </table>
                <br>
