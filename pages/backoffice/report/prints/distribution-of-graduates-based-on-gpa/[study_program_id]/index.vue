...
onMounted(async () => {
    ...
    await getReportGpaDistributionListData()
    await delay(1000)
    window.print()
})
...
const reportGpaDistributionLists = ref<GetReportGpaDistributionListData[]>([])
async function getReportGpaDistributionListData() {
    try {
        ...
        if (resp != null) {
            reportGpaDistributionLists.value = resp?.data
        }
    } catch (error) {
        alert(error)
    }
        ...
        <div class="nobreak">
            ...
            <div class="page-break">
                ...
                <div class="header">
                    ...
                    <h2 align="center">DISTRIBUSI LULUSAN BERDASARKAN IPK PER TAHUN AJARAN </h2>
                ...
                <table class="tabel-common" align="center" width="100%" v-if="reportGpaDistributionLists.length != 0">
                    <tbody>
                        <tr align="center" valign="middle">
                            <th rowspan="2">No</th>
                            <th rowspan="2">Tahun<br>Lulus</th>
                            <th colspan="2" nowrap="nowrap">IPK&nbsp;&lt;&nbsp;2.5</th>
                            <th colspan="2" nowrap="nowrap">IPK 2.5 - 3.0</th>
                            <th colspan="2" nowrap="nowrap"> IPK&nbsp;&gt;&nbsp;3.0</th>
                            <th rowspan="2">Total<br>Lulusan</th>
                            <th rowspan="2">IPK<br>Rata-rata</th>
                        </tr>
                        <tr align="center">
                            <th>Jumlah</th>
                            <th>%</th>
                        ...
                        <tr align="center" v-for="item, i in reportGpaDistributionLists">
                            <td nowrap="nowrap">{{ i + 1 }}</td>
                            <td nowrap="nowrap">{{ item.graduation_school_year }}</td>
                            <td nowrap="nowrap">{{ item.total_less_than_25 }}</td>
                            <td nowrap="nowrap">{{ item.percentage_less_than_25 }}</td>
                            <td nowrap="nowrap">{{ item.total_between_25_and_30 }}</td>
                            <td nowrap="nowrap">{{ item.percentage_between_25_and_30 }}</td>
                            <td nowrap="nowrap">{{ item.total_more_than_30 }}</td>
                            <td nowrap="nowrap">{{ item.percentage_more_than_30 }}</td>
                            <td nowrap="nowrap">{{ item.total_graduates }}</td>
                            <td nowrap="nowrap">{{ item.average_gpa }}</td>
                        </tr>
