...
async function semesterDetail() {
    try {
        const { response: resp, error: error } = await getSemesterDetail(semesterId.value)
        if (error) {
            let dataError = <ErrorRoot>error;
            alert(dataError.meta.message);
            return;
...
async function getSubject() {
    try {
        const { response: resp, error: error } = await getSubjectList(limit, 1, '', curriculum_ids.value, '', '', semester_package.value, semesterId.value, false, false)
        if (error) {
            let dataError = <ErrorRoot>error;
            alert(dataError.meta.message);
            return;
...
// async function selectedSemester(){
//     console.log("wadaw")
//     await getStudentNotSubmit()
//     console.log(studentNotSubmits.value)
// }
...
async function process(){
    for (let index = 0; index < subjects.value.length; index++) {
        let subject = subjects.value[index];
        for (let i = 0; i < class_ids.value.length; i++) {
            let class_id = class_ids.value[i];
            if(class_id.subject_id == subject.id){
                if(subject.total_lesson_plan < 14){
...
watch(semesterId, async function (value) {
    if (value != '') {
        curriculum_ids.value = []
        await getStudentNotSubmit(pagination.value.limit, 1)

        await semesterDetail()

        for (let index = 0; index < semester_detail.value.curriculums.length; index++) {
            if(semester_detail.value.curriculums[index].study_program_id == studyProgramId.value) {
                curriculum_ids.value.push(semester_detail.value.curriculums[index].curriculum_id)
            }
        }
        console.log(semester_detail.value)
    }
})
