...
import { deleteGradeComponent } from '~~/composables/api/grade-component/deleteGradeComponent';
...
const title = ref('Jenis Komponen Nilai')
...
const gradeComponents = ref<GetGradeComponentListData[]>([])
async function getData(limit: number, page: number, searchText: string, adminStudyProgram: GetAdminStudyProgramListData) {
    try {
        ...
        const { response: resp, error: error } = await getGradeComponentList(limit, page, searchText, selectedAdminStudyPrograms.value.id, '')
        if (error) {
        ...
        if (resp != null) {
            gradeComponents.value = resp?.data
...
const detailData = ref(<GetGradeComponentListData>{
})
function showItem(data: GetGradeComponentListData) {
    mode.value = 'show'
    detailData.value = data
    dialogShow.value = true
}

const updateGradeComponentId = ref('')
const updateData = ref(<GetGradeComponentListData>{
})

function editItem(data: GetGradeComponentListData) {
    mode.value = 'update'
...
function closeUpdate() {
    ...
    setTimeout(() => {
        updateGradeComponentId.value = ""
    }, 300)
}


const deletedData = ref(<GetGradeComponentListData>{})

function deleteItem(data: GetGradeComponentListData) {
    mode.value = 'delete'
...
async function deleteItm() {
    try {
        const { response: deleteResponse, error } = await deleteGradeComponent(deletedData.value)
        if (error) {
...
<template>
    <div>
        <div v-if="mode == 'show'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogShow" max-width="800">
                    <PagesGradeComponentDetail :getGradeComponentListData="detailData" />
                </v-dialog>
            </v-col>
        </div>
        <div v-if="mode == 'create'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogCreate" max-width="800">
                    <PagesGradeComponentCreate @reload="getData(limit, 1, search, selectedAdminStudyPrograms)"
                        @close="closeCreate()" :adminStudyProgram="selectedAdminStudyPrograms" />
                </v-dialog>
            </v-col>
        </div>
        <div v-if="mode == 'update'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogUpdate" max-width="800">
                    <PagesGradeComponentUpdate @reload="getData(limit, 1, search, selectedAdminStudyPrograms)"
                        @close="closeUpdate()" :adminStudyProgram="selectedAdminStudyPrograms"
                        :getGradeComponentListData="updateData" />
                </v-dialog>
            </v-col>
        </div>
                    ...
                    <v-card>
                        ...
                        <v-card-text>
                            <v-form ref="form" v-model="valid" lazy-validation>
                                ...
                                <v-row>
                                    <div>{{ deletedData.study_program_name }}</div>
                                </v-row>
                                <v-row>
                                    <h5>Kategori Mata Kuliah</h5>
                                </v-row>
                                <v-row>
                                    <div>{{ deletedData.subject_category_name }}</div>
                                </v-row>
                                <v-row>
                                    <h5>Nama Jenis Komponen Nilai</h5>
                                </v-row>
                                <v-row>
                                ...
                                <v-row>
                                    <div>{{ deletedData.is_active ? "Aktif" : "Tidak Aktif" }}</div>
                                </v-row>
                    ...
                    <v-row>
                        <v-col cols="12" lg="4" md="6">
                            <v-select variant="outlined" hide-details :items="adminStudyPrograms" item-value="id"
                                item-title="name" label="Jenis Jenjang Studi"
        ...
        <v-row>
            <v-col cols="12">
                <UiParentCard title="Daftar Jenis Komponen Nilai">
                    <v-row>
                        <v-col cols="12" lg="4" md="6">
                            <v-text-field density="compact" v-model="search" label="Cari" hide-details variant="outlined"
                                @input="getData(limit, 1, $event.target.value, selectedAdminStudyPrograms)"></v-text-field>
                        </v-col>
                        <v-col cols="12" lg="8" md="6" class="text-right">
                            <v-btn color="primary" flat class="ml-auto" @click="create()"
                                ...
                                </v-icon>Tambah Jenis Komponen Nilai
                            </v-btn>
                        </v-col>
                    </v-row>
                </UiParentCard>
        ...
        <v-table class="mt-5">
            <thead>
                <tr>
                    ...
                    <th class="text-subtitle-1 font-weight-semibold">Kategori Mata Kuliahh</th>
                    <th class="text-subtitle-1 font-weight-semibold">Nama Jenis</th>
                    <th class="text-subtitle-1 font-weight-semibold">Status</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="item, index in gradeComponents" :key="item.id">
