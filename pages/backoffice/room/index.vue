...
async function getData(limit: number, page: number, searchText: string, Sample: GetSampleListData) {
    ...
    //     const { response: resp, error: error } = await getRoomList(limit, page, searchText, selectedSamples.value.id)
    //     if (error) {
        ...
        <!-- <div v-if="mode == 'show'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogShow" max-width="800">
                    <PagesBackofficeRoomDetail :getRoomListData="detailData" />
                </v-dialog>
            </v-col>
        </div>
