...
const title = ref('Data Dosen Keluar')
...
const GetLecturerResignationLists = ref<GetLecturerResignationListData[]>([])
async function getData(limit: number, page: number) {
    try {
        const { response: resp, error: error } = await getLecturerResignationList(limit, page, search.value, selectedAdminStudyProgramLists.value.id, idNationalLecturer.value, selectedSemesterLists.value.id)
        if (error) {
        ...
        if (resp != null) {
            GetLecturerResignationLists.value = resp?.data
        ...
        <div v-if="mode == 'create'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialog" max-width="800">
                    <PagesBackofficeLecturerResignationCreate @reload="getData(limit, 1)" @close="close()"
        ...
        <SharedUiParentCard :title="title">
            <v-row>
                ...
                <v-col cols="12" lg="4" md="6">
                    ...
                    <v-select v-model="selectedSemesterLists.id" item-value="id" variant="outlined" hide-details
                        :items="SemesterLists" label="Semester" @update:model-value="(sl: any) => SemesterChanged(sl)">
            ...
            <v-row>
                <v-col cols="12" lg="12" md="12" class="text-right">
                    <v-btn color="primary" flat class="ml-auto" @click="create()"
                        ...
                        </v-icon>Tambah Dosen Keluar
                    </v-btn>

                    <a target="_blank"
                        :href="'/backoffice/lecturer/prints/lecturer-out-retired/' + selectedAdminStudyProgramLists.id + '/' + selectedSemesterLists.id">
                        <v-btn color="success float-right" flat class="ml-2 mb-2"
                            :disabled="selectedAdminStudyProgramLists.id == '' || selectedSemesterLists.id == ''">
                            <PrinterIcon />Cetak Dosen Keluar
                        </v-btn>
                    </a>
                </v-col>
            </v-row>
        ...
        <VTable class="mt-5">
            <thead>
                <tr>
                    ...
                    <th class="text-subtitle-1 font-weight-semibold">Nomor Surat</th>
                    <th class="text-subtitle-1 font-weight-semibold">Tanggal Keluar</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="item, index in GetLecturerResignationLists">
                    <td>{{ index + 1 }}</td>
                    <td>{{ item.id_national_lecturer }}</td>
                    <td>{{ item.name }}</td>
                    <td>{{ item.resignation_number }}</td>
                    <td>{{ moment(item.resign_date).format("DD MMM YYYY") }}</td>
