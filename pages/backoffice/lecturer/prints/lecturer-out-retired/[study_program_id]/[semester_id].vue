...
onMounted(async () => {
    ...
    await getLecturerResignationListData(10000000, 1)
    await delay(1000)
    window.print()
})
...
async function getLecturerResignationListData(limit: number, page: number) {
    try {
        const { response: resp, error: error } = await getLecturerResignationList(limit, page, "", "", "", semester_id.value)
        if (error) {
...
<template>
    <div class="page-portrait">
        <div class="nobreak">
            ...
            <h2 align="center">DAFTAR DOSEN KELUAR</h2>
            ...
            <table class="tabel-common" width="100%">
                <tbody>
                    ...
                    <tr v-for="ll, index in GetLecturerResignationLists">
                        <td align="center">{{ index + 1 }}</td>
                        <td align="center">{{ ll.id_national_lecturer }}</td>
