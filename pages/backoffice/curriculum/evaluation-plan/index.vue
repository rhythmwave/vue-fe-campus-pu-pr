...
const page = ref({ title: 'Daftar Rencana Pembelajaran' });
const title = ref('Daftar Rencana Pembelajaran')
...
async function getData(limit: number, page: number, search: string) {
    try {
        ...
        const { response: resp, error: error } = await getCuriculumList(20, 1, '', selectedAdminStudyPrograms.value.id)
        if (error) {
...
<template>
    <div>
        ...
        <v-row>
            <v-col cols="12">
                <UiParentCard title="Program Study">
                    <v-row>
