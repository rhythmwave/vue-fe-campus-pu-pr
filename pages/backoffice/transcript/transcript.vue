...
const page = ref({ title: 'Daftar Transkrip' });
const title = ref('Daftar Transkrip')
...
watch(from, async function (value) {
    if (value != '') {
        studentRequests.value.studentForceFrom = value
        await getData(limit, 1)
    }
})
watch(to, async function (value) {
    if (value != '') {
        studentRequests.value.studentForceTo = value
        await getData(limit, 1)
    }
})
                    ...
                    <v-row>
                        <v-col cols="12" lg="4" md="6">
                            <a :target="selectedAdminStudyPrograms.id ? '_blank' : ''"
                                ...
                                <v-btn color="primary" flat class="ml-2" :disabled="!selectedAdminStudyPrograms.id">
                                    Rekap Daftar Transkrip
                                </v-btn>
                            </a>
                        </v-col>
                    </v-row>
        ...
        <v-table class="mt-5">
            <thead>
                <tr>
                    ...
                    <!-- <th rowspan="2" class="text-subtitle-1 font-weight-semibold">Aksi</th> -->
                    ...
                    <th rowspan="2" class="text-subtitle-1 font-weight-semibold">Status MHS</th>
            ...
            <tbody>
                <tr v-for="item, index in students" :key="item.id">
                    ...
                    <td class="text-subtitle-1">{{ item.study_plan_total_mandatory_credit }}</td>
                    <td class="text-subtitle-1">{{ item.study_plan_total_optional_credit }}</td>
                    <td class="text-subtitle-1">{{ item.study_plan_total_mandatory_credit +
                        item.study_plan_total_optional_credit }}</td>
                        ...
                        <div class="d-flex align-center">
                            ...
                            <v-tooltip text="Detail">
                                <template #activator="{ props }">
                                    <a :href="'/backoffice/transcript/detail/' + item.id">
