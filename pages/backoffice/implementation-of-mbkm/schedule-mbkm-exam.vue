...
async function getSchedule(limit: number, page: number, search: string) {
    try {
        const { response: resp, error: error } = await getScheduleList(limit, page, classroom_id.value.id, false, is_exam.value, exam_type.value)
        if (error) {
            let dataError = <ErrorRoot>error;
            alert(dataError.meta.message);
            return;
...
async function examTypeOnChange(item: any) {
    if (item != '') {
        exam_type.value = item
    }
    if (exam_type.value != '') {
        await getSchedule(limit, page.value, search.value)
    }
...
<template>
    ...
    <div v-if="mode == 'edit'">
        <v-col cols="12" lg="8" md="6" class="text-right">
            <v-dialog v-model="dialog" max-width="800">
                <PagesBackofficeScheduleEditExam :isMbkm="true" :title="'Edit ' + title" @close="close"
                ...
                <v-row>
                    <v-col cols="12" lg="4" md="6">
                        <v-select variant="outlined" hide-details :items="adminStudyPrograms" item-value="id"
                            item-title="name" label="Pilih Program Studi" @update:model-value="(sl: any) => adminStudyProgramsOnChange(sl)" return-object></v-select>
                    </v-col>
                    <v-col cols="12" lg="4" md="6">
        ...
        <v-col cols="12">
            <UiParentCard :title="'Daftar ' + title">
                <v-row>
                    ...
                    <v-col cols="12" lg="4" md="4">
                        <v-select v-model="exam_type" variant="outlined" hide-details
                            :items="[{ id: 'midterm', name: 'UTS' }, { id: 'endterm', name: 'UAS' }]" item-value="id"
                            item-title="name" label="Pilih Tipe"
                            @update:model-value="(sl: any) => examTypeOnChange(sl)"></v-select>
                        <!-- <v-text-field density="compact" v-model="search" label="Cari" hide-details variant="outlined"
                            @input="getSchedule(limit, page, $event.target.value)"></v-text-field> -->
                    </v-col>
