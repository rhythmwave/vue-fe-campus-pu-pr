...
<template>
    ...
    <div v-if="mode == 'edit'">
        <!-- <v-col cols="12" lg="8" md="6" class="text-right">
            <v-dialog v-model="dialog" max-width="800">
                <PagesBackofficeScheduleEdit :title="'dit ' + title" :isMbkm="false" @close="close"
                    @reload="getClassroom(limit, 1, '', true)" :data-detail=scheduleDetail :data-study-program-id=study_program_id.id
                    :data-semester-id=semester_id.id />
            </v-dialog>
        </v-col> -->
    </div>
        ...
        <v-col cols="12">
            <UiParentCard :title="'Pilih ' + title">
                <v-row>
                    ...
                    <v-col cols="12" lg="4" md="6">
                        <!-- <v-select variant="outlined" hide-details :items="classrooms" item-value="id" item-title="name"
                            label="Pilih Kelas" @update:model-value="(sl: any) => classroomOnChange(sl)"></v-select> -->
        ...
        <tbody>
            <tr v-for="item, index in classrooms" :key="item.id">
                ...
                <td>
                    ...
                    <v-tooltip text="Show">
                        <template #activator="{ props }">
                            <NuxtLink :to="'/backoffice/schedule/classroom-schedule/'+item.id">
