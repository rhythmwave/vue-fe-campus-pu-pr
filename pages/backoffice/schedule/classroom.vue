...
async function getClassroom(limit: number, page: number, search: string, is_active: boolean) {
    try {
        const { response: resp, error: error } = await getClassroomList(limit, page, search, studyProgramId.value, semester_id.value, is_active, '', false)
        if (error) {
            let dataError = <ErrorRoot>error;
            alert(dataError.meta.message);
            return;
...
<template>
    ...
    <div v-if="mode == 'create'">
        <v-col cols="12" lg="8" md="6" class="text-right">
            <v-dialog v-model="dialog" max-width="800">
                <PagesBackofficeClassroomCreate :title="'Tambah ' + title" :studyProgramId="studyProgramId" @close="close"
                    @reload="getClassroom(limit, 1, '', is_active)" />
            </v-dialog>
        </v-col>
    </div>
    <div v-if="mode == 'show'">
    ...
    <div v-if="mode == 'edit'">
        <v-col cols="12" lg="8" md="6" class="text-right">
            <v-dialog v-model="dialog" max-width="800">
                <PagesBackofficeClassroomEdit :title="'Edit ' + title" :data-detail="classroomDetail" @close="close"
                    @reload="getClassroom(limit, 1, '', is_active)" />
            </v-dialog>
        </v-col>
    </div>
        ...
        <tr>
            <td colspan="3">
                <TablePaginationView :limit="pagination.limit" :page="pagination.page"
                    ...
                    @prevPage="getClassroom(pagination.limit, pagination.prev, '', is_active)"
                    @jumpPage="(page: any) => getClassroom(pagination.limit, page, '', is_active)"
                    @nextPage="getClassroom(pagination.limit, pagination.next, '', is_active)" />
            </td>
        </tr>
    </tfoot>
</v-table></template>