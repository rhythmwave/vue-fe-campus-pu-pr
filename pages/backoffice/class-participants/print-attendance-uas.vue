...
const page = ref({ title: 'Cetak Presensi UAS' });
...
async function getLectureData(limit: number, page: number) {
    try {
        const { response: resp, error: error } = await getLectureList(limit, page, "", false, true, "endterm")
        if (error) {
            let dataError = <ErrorRoot>error;
            alert(dataError.meta.message);
            return;
                    ...
                    <td>
                        <div class="d-flex align-center">
                            <v-tooltip text="Print">
                                <template v-slot:activator="{ props }">
                                    <a target="_blank" icon flat v-bind="props"
                                        :href="'/backoffice/class-participants/prints/uas/' + item.class_id">
                            ...
                            <v-tooltip text="Print Berita Acara">
                                <template v-slot:activator="{ props }">
                                    <a target="_blank" :href="'/backoffice/class-participants/prints/ba/' + item.class_id+'/endterm'">
