...
import { deleteRoom } from '~~/composables/api/room/deleteRoom';
...
const page = ref({ title: 'Manajemen Ruang/Kelas' });
const title = ref('Manajemen Ruang/Kelas')
...
async function getData(limit: number, page: number, searchText: string) {
    try {
        ...
        const { response: resp, error: error } = await getRoomList(limit, page, searchText, <string>route.query.id, false, '', '', '', '', '')
        if (error) {
...
const detailData = ref(<GetRoomListData>{
})
function showItem(data: GetRoomListData) {
    mode.value = 'show'
    detailData.value = data
    dialogShow.value = true
}

const updateRoomId = ref('')
const updateData = ref(<GetRoomListData>{
})

function editItem(data: GetRoomListData) {
    mode.value = 'update'
...
const deletedData = ref(<GetRoomListData>{})

function deleteItem(data: GetRoomListData) {
    mode.value = 'delete'
...
async function deleteItm() {
    try {
        const { response: deleteResponse, error } = await deleteRoom(deletedData.value)
        if (error) {
...
<template>
    <div>
        <div v-if="mode == 'show'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogShow" max-width="800">
                    <PagesBackofficeRoomDetail :getRoomListData="detailData" :getBuildingDetaiData="buildingDetailData"/>
                </v-dialog>
            </v-col>
        </div>
        <div v-if="mode == 'create'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogCreate" max-width="800">
                    <PagesBackofficeRoomCreate @reload="getData(limit, 1, search)" @close="closeCreate()" 
        ...
        <div v-if="mode == 'update'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogUpdate" max-width="800">
                    <PagesBackofficeRoomUpdate @reload="getData(limit, 1, search)" @close="closeUpdate()"
                        :getRoomListData="updateData" :getBuildingDetaiData="buildingDetailData"/>
                </v-dialog>
            </v-col>
        </div>
                ...
                <v-dialog v-model="dialogDelete" max-width="800">
                    <v-card>
                        <v-card-title class="pa-4 bg-secondary">
                            <span class="title text-white">Hapus Ruangan</span>
                        </v-card-title>
                        <v-card-text>
        ...
        <v-row>
            <v-col cols="12">
                <UiParentCard title="Daftar Ruangan">
                    <v-row>
                        ...
                        <v-col cols="12" lg="8" md="6" class="text-right">
                            ...
                            <nuxt-Link class="text-decoration-none color-inherits" :to="'/backoffice/space-management/room-management'">
