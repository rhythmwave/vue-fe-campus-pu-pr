...
import { deleteGraduationSession } from '~~/composables/api/graduation-session/deleteGraduationSession';
...
const page = ref({ title: 'Daftar Sesi Wisuda' });
const title = ref('Daftar Sesi Wisuda')
...
async function getData(limit: number, page: number, searchText: string) {
    try {
        ...
        const { response: resp, error: error } = await getGraduationSessionList(limit, page, searchText)
        if (error) {
...
const detailData = ref(<GetGraduationSessionListData>{
})
function showItem(data: GetGraduationSessionListData) {
    mode.value = 'show'
    detailData.value = data
    dialogShow.value = true
}

const updateGraduationSessionId = ref('')
const updateData = ref(<GetGraduationSessionListData>{
})

function editItem(data: GetGraduationSessionListData) {
    mode.value = 'update'
...
function closeUpdate() {
    ...
    setTimeout(() => {
        updateGraduationSessionId.value = ""
    }, 300)
}


const deletedData = ref(<GetGraduationSessionListData>{})

function deleteItem(data: GetGraduationSessionListData) {
    mode.value = 'delete'
...
async function deleteItm() {
    try {
        const { response: deleteResponse, error } = await deleteGraduationSession(deletedData.value)
        if (error) {
...
<template>
    <div>
        <div v-if="mode == 'show'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogShow" max-width="800">
                    <PagesBackofficeGraduationSessionDetail :getGraduationSessionListData="detailData" />
                </v-dialog>
            </v-col>
        </div>
        <div v-if="mode == 'create'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogCreate" max-width="800">
                    <PagesBackofficeGraduationSessionCreate @reload="getData(limit, 1, search)" @close="closeCreate()" />
                </v-dialog>
            </v-col>
        </div>
        <div v-if="mode == 'update'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogUpdate" max-width="800">
                    <PagesBackofficeGraduationSessionUpdate @reload="getData(limit, 1, search)" @close="closeUpdate()"
                        :getGraduationSessionListData="updateData" />
                </v-dialog>
            </v-col>
        </div>
                ...
                <v-dialog v-model="dialogDelete" max-width="800">
                    <v-card>
                        <v-card-title class="pa-4 bg-secondary">
                            <span class="title text-white">Hapus Sesi Wisuda</span>
                        </v-card-title>
                        <v-card-text>
                            <v-form ref="form" v-model="valid" lazy-validation>
                                ...
                                <v-row>
                                    <div>
                                        {{ convertDayOfWeek(new Date(deletedData.session_date).getDay()) }},
                                        {{ new Date(deletedData.session_date).getDate() }},
                                        {{ convertMonthToTextString(new Date(deletedData.session_date).getMonth() + 1) }}
                                        {{ new Date(deletedData.session_date).getFullYear() }}
                                    </div>
                                </v-row>
                                <v-row>
                                    <h5>Nomor Sesi</h5>
                                ...
                                <v-row>
                                    <div>{{ deletedData.session_number }}</div>
                                </v-row>
                                <v-row>
                                    <h5>Tahun</h5>
                                </v-row>
                                <v-row>
                                    <div>{{ deletedData.session_school_year }}</div>
                                </v-row>
        ...
        <v-row>
            <v-col cols="12">
                <UiParentCard title="Daftar Sesi Wisuda">
                    <v-row>
                        ...
                        <v-col cols="12" lg="8" md="6" class="text-right">
                            <v-btn color="primary" flat class="ml-auto" @click="create()">
                                ...
                                </v-icon>Tambah Sesi Wisuda
                            </v-btn>

                            <a target="_blank" :href="'/backoffice/graduation/prints/graduation-session'">
        ...
        <v-table class="mt-5">
            ...
            <tbody>
                <tr v-for="item, index in graduationSessions" :key="item.id">
                    ...
                    <td class="text-subtitle-1">{{ item.session_number }}</td>
                    <td class="text-subtitle-1">{{ item.session_school_year }}</td>
                    <td class="text-subtitle-1">
                        <!-- {{ convertDayOfWeek(new Date(item.session_date).getDay()) }}, -->
                        {{ new Date(item.session_date).getDate() }},
                        {{ convertMonthToTextString(new Date(item.session_date).getMonth() + 1) }}
                        {{ new Date(item.session_date).getFullYear() }}
                    </td>
                </tr>
            </tbody>
            <tfoot>
