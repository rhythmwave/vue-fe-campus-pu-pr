...
import { deleteAdminStudyProgram } from '~~/composables/api/admin-study-program/deleteAdminStudyProgram';
...
const page = ref({ title: 'Daftar Gelar' });
const title = ref('Daftar Gelar')
...
async function getData(limit: number, page: number, searchText: string) {
    try {
        ...
        const { response: resp, error: error } = await getAdminStudyProgramList(limit, page, searchText, '')
        if (error) {
...
// const detailData = ref(<GetAdminStudyProgramListData>{
// })
// function showItem(data: GetAdminStudyProgramListData) {
//     mode.value = 'show'
...
const updateData = ref(<GetAdminStudyProgramListData>{
})

function editItem(data: GetAdminStudyProgramListData) {
    mode.value = 'update'
...
// const deletedData = ref(<GetAdminStudyProgramListData>{})

// function deleteItem(data: GetAdminStudyProgramListData) {
//     mode.value = 'delete'
...
//     try {
//         const { response: deleteResponse, error } = await deleteAdminStudyProgram(deletedData.value)
//         if (error) {
...
<template>
    <div>
        <!-- <div v-if="mode == 'show'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogShow" max-width="800">
                    <PagesBackofficeAdminStudyProgramDetail :getAdminStudyProgramListData="detailData" />
                </v-dialog>
            </v-col>
        </div> -->
        <!-- <div v-if="mode == 'create'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogCreate" max-width="800">
                    <PagesBackofficeAdminStudyProgramCreate @reload="getData(limit, 1, search)" @close="closeCreate()"
        ...
        <div v-if="mode == 'update'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogUpdate" max-width="800">
                    <PagesBackofficeDegreeUpdate @reload="getData(limit, 1, search)" @close="closeUpdate()"
                        :getAdminStudyProgramDegreeListData="updateData" />
                </v-dialog>
            </v-col>
        </div>
                ...
                <v-dialog v-model="dialogDelete" max-width="800">
                    <v-card>
                        <v-card-title class="pa-4 bg-secondary">
                            <span class="title text-white">Hapus Gelar</span>
                        </v-card-title>
                        <v-card-text>
        ...
        <v-row>
            <v-col cols="12">
                <UiParentCard title="Daftar Gelar">
                    <v-row>
        ...
        <v-table class="mt-5">
            <thead>
                <tr>
                    ...
                    <th class="text-subtitle-1 font-weight-semibold">Gelar Singkat</th>
                    <th class="text-subtitle-1 font-weight-semibold">Gelar Lengkap</th>
                    <th class="text-subtitle-1 font-weight-semibold">Gelar Asing</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="item, index in adminStudyPrograms" :key="item.id">
                    ...
                    <td class="text-subtitle-1">{{ item.short_degree }}</td>
                    <td class="text-subtitle-1">{{ item.degree }}</td>
                    <td class="text-subtitle-1">{{ item.english_degree }}</td>
                </tr>
            </tbody>
            <tfoot>
                <tr>
