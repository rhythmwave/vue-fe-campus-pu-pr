...
import { GetGraduationStudentListData, GetGraduationStudentListRequest } from '~~/types/api/graduation/GetGraduationStudentList';
import { deleteStudent } from '~~/composables/api/student/deleteStudent';
import { getGraduationStudentList } from '~~/composables/api/graduation/getGraduationStudentList';
...
const page = ref({ title: 'Pendaftaran Wisudawan' });
const title = ref('Pendaftaran Wisudawan')
...
onMounted(async () => {
    ...
    await getGraduationSessionListData();
    await getData(limit, 1)
})

const students = ref(<GetGraduationStudentListData[]>[])
const graduationStudentRequests = ref(<GetGraduationStudentListRequest>{
    studyProgramId: '',
    graduationSessionId: '',
})
const from = ref('')
...
async function getData(limit: number, page: number) {
    try {
        const { response: resp, error: error } = await getGraduationStudentList(limit, page, graduationStudentRequests.value)

        if (selectedAdminStudyPrograms.value.id == '' || selectedGetGraduationSessionLists.value.id == '') {
...
const GetGraduationSessionLists = ref<GetGraduationSessionListData[]>([])
const selectedGetGraduationSessionLists = ref(<GetGraduationSessionListData>{
    id: '',
    session_school_year: '',
    session_number: 0,
    session_year: 0,
...
async function getGraduationSessionListData() {
    try {
        ...
        if (resp != null) {
            GetGraduationSessionLists.value = resp?.data
        }
    } catch (error) {
        alert(error)
    }
...
const detailData = ref(<GetGraduationStudentListData>{
})
function showItem(data: GetGraduationStudentListData) {
    mode.value = 'show'
...
const updateData = ref(<GetGraduationStudentListData>{
})

function editItem(data: GetGraduationStudentListData) {
    mode.value = 'update'
...
const deletedData = ref(<GetGraduationStudentListData>{})

function deleteItem(data: GetGraduationStudentListData) {
    mode.value = 'delete'
...
function AdminStudyProgramsOnChange(id: string) {
    ...
    graduationStudentRequests.value.studyProgramId = selectedAdminStudyPrograms.value.id
    getData(limit, 1)
}
function GraduationSessionsOnChange(id: string) {
    selectedGetGraduationSessionLists.value = <GetGraduationSessionListData>GetGraduationSessionLists.value.find((x) => x.id == id)
    graduationStudentRequests.value.graduationSessionId = selectedGetGraduationSessionLists.value.id
    getData(limit, 1)
}
...
<template>
    <div>
        <div v-if="mode == 'show'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogShow" max-width="800">
                    <PagesBackofficeGraduationDetail :getGraduationStudentListData="detailData" />
                </v-dialog>
            </v-col>
        </div>
        <div v-if="mode == 'create'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogCreate" max-width="800">
                    <PagesBackofficeGraduationApply @reload="getData(limit, 1)" @close="closeCreate()" />
                </v-dialog>
            </v-col>
        </div>
            ...
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogUpdate" max-width="800">
                    <PagesBackofficeStudentUpdate @reload="getData(limit, 1)" @close="closeUpdate()"
                        :getGraduationStudentListData="updateData" />
                </v-dialog>
            </v-col>
        </div>
                ...
                <v-dialog v-model="dialogDelete" max-width="800">
                    <v-card>
                        <v-card-title class="pa-4 bg-secondary">
                            <span class="title text-white">Hapus Pendaftaran Wisudawan</span>
                        </v-card-title>
                        <v-card-text>
        ...
        <v-row>
            <v-col cols="12">
                <UiParentCard title="Pencarian Pendaftaran Wisudawan">
                    <v-row>
                        <v-col cols="12" lg="4" md="6">
                            <v-label style="margin-bottom: 5px;">Sesi Wisuda</v-label>
                            <v-select variant="outlined" hide-details :items="GetGraduationSessionLists" item-value="id"
                                item-title="session_school_year" label="Sesi Wisuda"
                                @update:model-value="(sl: any) => GraduationSessionsOnChange(sl)"></v-select>
                        </v-col>
                        <v-col cols="12" lg="4" md="6">
                        ...
                        <div class="d-flex align-center">
                            ...
                            <v-tooltip text="Ubah">
                                <template #activator="{ props }">
                                    <a target="_blank" :href="'/backoffice/transcript/prints/transcript/' + item.id">
