...
import { deleteAcademicGuidance } from '~~/composables/api/academic-guidance/deleteAcademicGuidance';
import { getAcademicGuidanceList } from '~~/composables/api/academic-guidance/getAcademicGuidanceList';
...
const academicGuidances = ref<GetAcademicGuidanceListData[]>([])
async function getData(limit: number, page: number, searchText: string, Sample: GetSampleListData) {
    try {
        ...
        const { response: resp, error: error } = await getAcademicGuidanceList(limit, page, searchText, selectedSamples.value.id)
        if (error) {
        ...
        if (resp != null) {
            academicGuidances.value = resp?.data
...
const detailData = ref(<GetAcademicGuidanceListData>{
})
function showItem(data: GetAcademicGuidanceListData) {
    mode.value = 'show'
...
const updateData = ref(<GetAcademicGuidanceListData>{
})
...
const deletedData = ref(<GetAcademicGuidanceListData>{})

function deleteItem(data: GetAcademicGuidanceListData) {
    mode.value = 'delete'
...
async function deleteItm() {
    try {
        const { response: deleteResponse, error } = await deleteAcademicGuidance(deletedData.value)
        if (error) {
...
<template>
    <div>
        <div v-if="mode == 'show'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogShow" max-width="800">
                    <PagesBackofficeAcademicGuidanceDetail :getAcademicGuidanceListData="detailData" />
                </v-dialog>
            </v-col>
        </div>
        <div v-if="mode == 'create'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogCreate" max-width="800">
                    <PagesBackofficeAcademicGuidanceCreate @reload="getData(limit, 1, search, selectedSamples)"
        ...
        <div v-if="mode == 'update'">
            <v-col cols="12" lg="8" md="6" class="text-right">
                <v-dialog v-model="dialogUpdate" max-width="800">
                    <PagesBackofficeAcademicGuidanceUpdate @reload="getData(limit, 1, search, selectedSamples)"
                        @close="closeUpdate()" :Sample="selectedSamples"
                        :getAcademicGuidanceListData="updateData" />
                </v-dialog>
            </v-col>
        </div>
        ...
        <v-table class="mt-5">
            ...
            <tbody>
                <tr v-for="item, index in academicGuidances" :key="item.id">
