...
const page = ref({ title: 'Daftar Semester' });
const title = ref('Daftar Semester')
...
async function getData(limit: number, page: number) {
    try {
        const { response: resp, error: error } = await getSemesterList(limit, page, '', '')
        if (error) {
...
<template>
    <div>
                ...
                <v-dialog v-model="dialogDelete" max-width="800">
                    <v-card>
                        <v-card-title class="pa-4 bg-secondary">
                            <span class="title text-white">Hapus Semester</span>
                        </v-card-title>
                        <v-card-text>
        ...
        <v-row>
            <v-col cols="12">
                <UiParentCard title="Daftar Semester">
                    <v-row>
                ...
                <tr v-for="item, index in semesters" :key="item.id">
                    ...
                    <td>
                        <div class="d-flex align-center">
                            ...
                            <v-tooltip text="Activate" v-if="!item.is_active">
                                <template #activator="{ props }">
                                    <v-btn icon flat v-bind="props" @click="activateItem(item)">
                                        <LockOpenIcon stroke-width="1.5" size="20" class="text-primary" />
                                    </v-btn>
                                </template>
                            </v-tooltip>
                        </div>
                    ...
                    <td class="text-subtitle-1">{{ item.semester_start_year }} - {{ item.semester_type }}</td>
