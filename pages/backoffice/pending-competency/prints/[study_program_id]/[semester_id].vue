...
onMounted(async () => {
    ...
    await getStudentData(10000000, 1)
    await delay(1000)
    window.print()
})
...
const GetStudents = ref<GetStudentListData[]>([])
async function getStudentData(limit: number, page: number) {
    try {
        const { response: resp, error: error } = await getStudentList(limit, page, <GetStudentListRequest>{
            ...
            hasStudyPlan: '',
            statusSemesterId: semester_id.value,
            isRegistered: '',
            hasPaid: '',
            isGraduateEligible: '',
            isThesisEligible: '',
...
<template>
    <div class="page-portrait">
        <div class="nobreak">
            ...
            <h2 align="center">Daftar Mahasiswa Menunggu Uji Kompotensi</h2>
            ...
            <table class="tabel-common" width="100%">
                <tbody>
                    ...
                    <tr v-for="ll, index in GetStudents">
                        <td align="center">{{ index + 1 }}</td>
                        <td align="center">{{ ll.nim_number }}</td>
                        <td align="center">{{ ll.name }}</td>
                        <td align="center">{{ ll.student_force }}</td>
                        <td align="center">{{ studyPrograms.name }}</td>
                    </tr>
