...
import { deleteLecturerClassMaterial } from '~~/composables/api/lecturer-class-material/deleteLecturerClassMaterial';
...
const editData = ref(<GetLecturerClassMaterialListData>{})
function edit(data: GetLecturerClassMaterialListData) {
  mode.value = 'edit'
  dialog.value = true
  editData.value = data
}
...
async function lecturerAssignedClassOnChange(id: any) {
  ...
  getLecturerClassMaterialData(limit, 1)
}
const lecturerClassMaterials = ref<GetLecturerClassMaterialListData[]>([])
async function getLecturerClassMaterialData(limit: number, page: number) {
  try {
    ...
    const { response: resp, error: error } = await getLecturerClassMaterialList(limit, page, search.value, selectedLecturerAssignedClass.value.id)
    if (error) {
      let dataError = <ErrorRoot>error;
      alert(dataError.meta.message);
      return;
...
async function studentStudentClassOnChange(id: any) {
  ...
  getStudentClassMaterialData(limit, 1)
}
const studentClassMaterials = ref<GetStudentClassMaterialListData[]>([])
async function getStudentClassMaterialData(limit: number, page: number) {
  try {
    ...
    const { response: resp, error: error } = await getStudentClassMaterialList(limit, page, search.value, selectedStudentStudentClass.value.id)
    if (error) {
      let dataError = <ErrorRoot>error;
      alert(dataError.meta.message);
      return;
...
async function deleteItm() {
  try {
    const { response: deleteResponse, error } = await deleteLecturerClassMaterial(deleteList.value)
    if (error) {
  ...
  <div v-if="mode == 'create'">
    <v-col cols="12" lg="8" md="6" class="text-right">
      <v-dialog v-model="dialog" max-width="800">
        <PagesAcademicPortalVirtualClassMaterialCreate :semester="selectedLecturerSemester" @close="close"
          @reload="getLecturerClassMaterialData(limit, 1)" />
      </v-dialog>
    </v-col>
  </div>
  <div v-if="mode == 'edit'">
    <v-col cols="12" lg="8" md="6" class="text-right">
      <v-dialog v-model="dialog" max-width="800">
        <PagesAcademicPortalVirtualClassMaterialUpdate :detail="editData" :semester="selectedLecturerSemester"
          @close="close" @reload="getLecturerClassMaterialData(limit, 1)" />
      </v-dialog>
    </v-col>
  </div>
      ...
      <v-dialog v-model="dialog" max-width="800">
        <v-card>
          <v-card-title class="pa-4 bg-secondary">
            <span class="title text-white">Hapus Materi Kuliah </span>
          </v-card-title>
          <v-card-text>
        ...
        <v-row>
          ...
          <v-col cols="12" lg="4" md="4">
            <v-text-field density="compact" v-model="search" label="Judul" hide-details variant="outlined"
              @input="userData.app_type == 'lecturer' ? getLecturerClassMaterialData(limit, 1) : getStudentClassMaterialData(limit, 1)"></v-text-field>
          </v-col>
        </v-row>
      </UiParentCard>
    </v-col>
  ...
  <v-table class="mt-5">
    <thead>
      <tr>
        ...
        <th class="text-subtitle-1 font-weight-semibold" v-if="userData.app_type == 'lecturer'">Status</th>
    ...
    <tbody>
      <tr v-for="material, i in lecturerClassMaterials" v-if="userData.app_type == 'lecturer'">
        <td>
          <v-checkbox :value="material.id" v-model="deleteList"></v-checkbox>
        </td>
        <td>{{ i + 1 }}</td>
        <td>{{ selectedLecturerAssignedClass.name }}</td>
        <td>{{ material.title }}</td>
        <td>{{ material.is_active ? "Aktif" : "Tidak Aktif" }}</td>
        <td>
          <NuxtLink :href="material.file_url" v-if="material.file_url" target="_blank" download>
          ...
          <div class="d-flex align-center">
            <v-tooltip text="Ubah">
              <template #activator="{ props }">
                <v-btn icon flat @click="edit(material)">
      ...
      <tr v-for="material, i in studentClassMaterials" v-if="userData.app_type == 'student'">
        <td>
          <v-checkbox :value="material.id" v-model="deleteList"></v-checkbox>
        </td>
        <td>{{ i + 1 }}</td>
        <td>{{ selectedStudentStudentClass.name }}</td>
        <td>{{ material.title }}</td>
