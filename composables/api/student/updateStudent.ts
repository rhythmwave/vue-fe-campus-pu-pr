...
import { UpdateStudentRequest, UpdateStudentRoot } from "~~/types/api/student/UpdateStudent";
import { convertDateWithoutTimeToTimezoned } from '~~/utils/helpers/date-utils';

export async function updateStudent(body: UpdateStudentRequest)
    : Promise<{ response: UpdateStudentRoot | null, error: ErrorRoot | null }> {
    try {
        var req = Object.assign({}, body)
        const { data: data, pending, refresh, error } = await useAPIFetch<UpdateStudentRoot>(() => `/api/v1/admin/student/admin_student.AdminStudentHandler/Update`, <any>{
            method: 'PATCH',
            body: {
                id: req.id,
                name: req.name,
