...
import { CreateExamSupervisor, CreateExamSupervisorRoot } from "~~/types/api/class-participant/CreateExamSupervisor";
import { convertDateWithoutTimeToTimezoned } from '~~/utils/helpers/date-utils';

export async function createExamSupervisorRole(body: CreateExamSupervisor)
    : Promise<{ response: CreateExamSupervisorRoot | null, error: ErrorRoot | null }> {
    try {
        var req = Object.assign({}, body)
        console.log(req.birth_date)
        const { data: data, pending, refresh, error } = await useAPIFetch<CreateExamSupervisorRoot>(() => `/api/v1/admin/student_class/admin_exam_supervisor.AdminExamSupervisorHandler/Create`, <any>{
            method: 'POST',
            body: {
