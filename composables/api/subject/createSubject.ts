...
import { CreateSubjectRequest, CreateSubjectRoot } from "~~/types/api/subject/CreateSubject";

export async function createSubject(body: CreateSubjectRequest)
    : Promise<{ response: CreateSubjectRoot | null, error: ErrorRoot | null }> {
    try {
        var req = Object.assign({}, body)
        const { data: data, pending, refresh, error } = await useAPIFetch<CreateSubjectRoot>(() => `/api/v1/admin/curriculum/admin_subject.AdminSubjectHandler/Create`, <any>{
            method: 'POST',
            body: {
