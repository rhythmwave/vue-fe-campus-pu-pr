...
export async function createClassroom(body: CreateClassroomRequest)
    ...
    try {
        ...
        const { data: data, pending, refresh, error } = await useAPIFetch<ResponseInsertClassroom>(() => `/api/v1/admin/schedule/admin_class.AdminClassHandler/Create`, <any>{
            method: 'POST',
            body: req,
        })
        if (error.value) {
